# The SGM model in this file is very similar to that in sgm_symL.py.
# The only diff is we make the L hermitian
# Mar 29, 2023
import numpy as np
...
def network_transfer_local(C, D, parameters, w):
    ...
    Cc = np.triu(Cc) + np.tril(np.conj(Cc), -1) # make Cc hermitian
