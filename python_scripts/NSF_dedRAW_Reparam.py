...
def _thin_psd_mat(psd_mat):
    ...
    del_idxs = np.arange(int(length/2)+1, length, 1)
...
# paras  for this file
_paras = edict()
_folder_path = f"./generalNoiseDedRawReparam_{paras.num_prior_sps:.0f}" +               f"_sd{paras.noise_sd*100:.0f}" +               f"_denest{paras.den_est}" +               f"_embed{paras.is_embed}"
